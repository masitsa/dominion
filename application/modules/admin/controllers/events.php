...
class Events extends admin {
	...
	*
	*/
	public function index($order = 'event_start_time', $order_method = 'DESC')
	{
		$where = 'event_id > 0';
		$table = 'event';
		$segment = 4;
		...
		$config['base_url'] = site_url().'events/'.$order.'/'.$order_method;
		...
		$query = $this->event_model->get_all_events($table, $where, $config["per_page"], $page, $order, $order_method);
	...
	function add_event()
	{
		...
		$this->form_validation->set_rules('event_name', 'Title', 'required|trim|xss_clean');
		$this->form_validation->set_rules('event_description', 'Description', 'required|trim|xss_clean');
		$this->form_validation->set_rules('event_start_date', 'Start date', 'required|xss_clean');
		$this->form_validation->set_rules('event_start_time', 'Start time', 'required|xss_clean');
		$this->form_validation->set_rules('event_end_date', 'Start date', 'required|xss_clean');
		$this->form_validation->set_rules('event_end_time', 'Start time', 'required|xss_clean');

		if ($this->form_validation->run())
		{	
			if(empty($event_error))
			{
				$start = $this->input->post("event_start_date").' '.$this->input->post("event_start_time").':00';
				$end = $this->input->post("event_end_date").' '.$this->input->post("event_end_time").':00';
		...
		$data['title'] = $v_data['title'] = 'Add Event';
	...
	function edit_event($event_id, $page)
	{
		...
		$data['title'] = $v_data['title'] = 'Edit Event';
