		...
		if ($query->num_rows() > 0)
		{
			...
			foreach ($query->result() as $row)
			{
				...
				else if($blog_category_status == 1)
				{
					...
					$button = '<a class="btn btn-default" href="'.site_url().'deactivate-blog-category/'.$blog_category_id.'" onclick="return confirm(\'Do you want to deactivate '.$blog_category_name.'?\');">Deactivate</a>';
				}
				...
				'
					<tr>
						...
						<td>'.$posts.'</td>
						<td>'.$status.'</td>
						<td><a href="'.site_url().'edit-blog-category/'.$blog_category_id.'" class="btn btn-sm btn-primary">Edit</a></td>
						<td>'.$button.'</td>
						...
						<section class="panel">
							...
							<div class="panel-body">
                            	<div class="row" style="margin-bottom:20px;">
                                    <div class="col-lg-12">
                                    	<a href="<?php echo site_url().'add-blog-category';?>" class="btn btn-success pull-right">Add Category</a>
                                    </div>
                                </div>
                                ...
                                if ($query->num_rows() > 0)
                                {
                                    echo '<div class="table-responsive">'.$result.'</div>';
                                }
                                ?>
